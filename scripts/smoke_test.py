...
def run(
    args: list[str], env: dict[str, str], expected=(0,), timeout=60
        ...
        try:
            ...
            run(
                [
                    ...
                    url + "/ok",
                    "--column",
                    "smoke=header:X-Smoke",
                    "--quiet",
                    "--jsonl",
                    str(root / "request.jsonl"),
                ],
                env,
            )
            run(
            ...
            run(
                [
                    ...
                    "--quiet",
                    "--csv",
                    str(root / "intrude.csv"),
                    "--jsonl",
                    str(root / "intrude.jsonl"),
                ],
                env,
            )
            ...
            run(
                ["import", "raw", str(raw), "--output", str(root / "imported.json")],
                env,
            )
            ...
            run(
                ["request", "--session", "lab", "--url", url + "/session", "--quiet"],
                env,
            )
                ...
                json.dumps(
                    {
                        "steps": [
                            ...
                            {
                                "request": {"url": url + "/check?token={{TOKEN}}"},
                                "require_status": 200,
                            },
                        ]
                    }
            ...
            ws_thread = threading.Thread(
                target=start_websocket, args=(ws_port, ws_ready, ws_stop), daemon=True
            )
            ws_thread.start()
            ws_ready.wait(5)
            ...
            with urllib.request.urlopen(
                f"http://127.0.0.1:{web_port}/health", timeout=5
            ) as response:
