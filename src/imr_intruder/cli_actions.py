...
from .storage import (create_session, create_workspace, current_workspace, delete_session,
    export_workspace, list_sessions, list_workspaces, load_session, save_session,
...
def cmd_session(args: argparse.Namespace) -> int:
    if args.session_action == "create":
        create_session(args.name); console.print(f"Created session {args.name}")
        ...
        if not args.show_secrets:
            for key in ("auth", "cookies"):
                if data.get(key): data[key] = "<REDACTED>"
        _json_print(data)
        ...
        save_session(args.name, data); console.print(f"Updated cookies for {args.name}")
    return 0
...
def cmd_workspace(args: argparse.Namespace) -> int:
    if args.workspace_action == "create":
        ...
        if not name: raise ValueError("No workspace selected.")
        _json_print({"name": name, "path": str(workspace_path(name)), "current": name == current_workspace()})
...
def cmd_report(args: argparse.Namespace) -> int:
    ...
    console.print(str(output)); return 0
...
def cmd_macro(args: argparse.Namespace) -> int:
    ...
    _json_print(results); return 1 if any(item.get("error") for item in results) else 0
...
def cmd_browser(args: argparse.Namespace) -> int:
    _json_print(fetch_page(args.url, Path(args.screenshot) if args.screenshot else None, int(args.timeout * 1000))); return 0


def cmd_plugins(args: argparse.Namespace) -> int:
    _json_print(plugin_status()); return 0


def cmd_collab(args: argparse.Namespace) -> int:
    if args.collab_action == "create-token":
        token = create_token(args.name, args.role); console.print(token)
    elif args.collab_action == "list":
        _json_print(list_tokens())
    elif args.collab_action == "revoke":
        revoke_token(args.name); console.print(f"Revoked {args.name}")
    return 0
...
def cmd_web(args: argparse.Namespace) -> int:
    ...
    if action == "start":
        if args.background:
            _json_print(start_background(args.host, args.port, args.token, args.allow_remote, args.multiuser))
            return 0
...
def cmd_check_update(args: argparse.Namespace) -> int:
    ...
    _json_print(info.__dict__) if args.json_output else console.print(f"Current: {info.current}\nLatest: {info.latest}\nAvailable: {info.available}\nSource: {info.source}")
...
def cmd_update(args: argparse.Namespace) -> int:
    ...
    if not info.available and not args.force:
        console.print("Already up to date."); return 0
    _json_print(install_update(info, token=args.token, dry_run=args.dry_run)); return 0
