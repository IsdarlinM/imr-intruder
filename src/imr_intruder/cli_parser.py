...
def add_http_options(parser: argparse.ArgumentParser, require_url: bool = True) -> None:
    ...
    parser.add_argument("--method", "-X", type=str.upper, choices=["GET", "POST", "PUT", "PATCH", "DELETE", "HEAD", "OPTIONS"], default="GET")
    ...
    parser.add_argument("--timeout", type=float_range(0.0, 300.0, exclusive_minimum=True), default=15)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    repeater.add_argument("--repeat", type=int_range(1, 10000), default=1); add_execution_options(repeater); repeater.set_defaults(func=cmd_repeater)
    ...
    websocket = sub.add_parser("websocket", help="Send controlled WebSocket messages"); websocket.add_argument("url"); websocket.add_argument("--message", action="append"); websocket.add_argument("--messages-file"); websocket.add_argument("--header", action="append", default=[]); websocket.add_argument("--timeout", type=float_range(0.0, 300.0, exclusive_minimum=True), default=10); websocket.set_defaults(func=cmd_websocket)
    browser = sub.add_parser("browser", help="Render a page using optional Playwright"); browser.add_argument("url"); browser.add_argument("--screenshot"); browser.add_argument("--timeout", type=float_range(0.0, 600.0, exclusive_minimum=True), default=30); browser.set_defaults(func=cmd_browser)
    ...
    web.add_argument("--host", default="127.0.0.1"); web.add_argument("--port", type=int_range(1, 65535), default=7415); web.add_argument("--token"); web.add_argument("--allow-remote", action="store_true"); web.add_argument("--multiuser", action="store_true"); web.add_argument("--background", action="store_true"); web.add_argument("--no-browser", action="store_true"); web.set_defaults(func=cmd_web)
