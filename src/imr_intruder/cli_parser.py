...
from .cli_runtime import cmd_batch, cmd_intrude, cmd_request
from .core import validate_http_method
...
def float_range(minimum: float, maximum: float, *, exclusive_minimum: bool = False):
    def parse(value: str) -> float:
        converted = float(value)
        valid_minimum = converted > minimum if exclusive_minimum else converted >= minimum
        if not valid_minimum or converted > maximum:
            operator = ">" if exclusive_minimum else ">="
            raise argparse.ArgumentTypeError(f"must be {operator} {minimum} and <= {maximum}")
        return converted
...
def add_http_options(parser: argparse.ArgumentParser, require_url: bool = True) -> None:
    ...
    parser.add_argument("--method", "-X", type=validate_http_method, default="GET")
    ...
    parser.add_argument(
        "--timeout", type=float_range(0.0, 300.0, exclusive_minimum=True), default=15
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    intrude.add_argument("--max-requests", type=int_range(1, 100000), default=10000)
    ...
    repeater.add_argument("request_file")
    repeater.add_argument("--kind", choices=["raw", "curl", "har", "burp", "zap"], default="raw")
    repeater.add_argument("--repeat", type=int_range(1, 10000), default=1)
    add_execution_options(repeater)
    repeater.set_defaults(func=cmd_repeater)

    importer = sub.add_parser("import", help="Import HTTP requests")
    importer.add_argument("kind", choices=["raw", "curl", "har", "burp", "zap"])
    importer.add_argument("input")
    importer.add_argument("--output", "-o", required=True)
    importer.set_defaults(func=cmd_import)

    session = sub.add_parser("session", help="Manage persistent HTTP sessions")
    ...
    for action in ("create", "show", "delete"):
        ...
        if action == "show":
            child.add_argument("--show-secrets", action="store_true")
    child = session_sub.add_parser("list")
    child.add_argument("--json", dest="json_output", action="store_true")
    child = session_sub.add_parser("set")
    child.add_argument("name")
    child.add_argument("key")
    child.add_argument("value")
    child = session_sub.add_parser("cookies")
    child.add_argument("name")
    child.add_argument("--cookie", action="append", required=True)
    session.set_defaults(func=cmd_session)

    workspace = sub.add_parser("workspace", help="Manage isolated workspaces")
    workspace_sub = workspace.add_subparsers(dest="workspace_action", required=True)
    child = workspace_sub.add_parser("create")
    child.add_argument("name")
    child = workspace_sub.add_parser("list")
    child.add_argument("--json", dest="json_output", action="store_true")
    child = workspace_sub.add_parser("use")
    child.add_argument("name")
    child = workspace_sub.add_parser("show")
    child.add_argument("name", nargs="?")
    child = workspace_sub.add_parser("export")
    child.add_argument("name")
    ...
    report.add_argument("--title", default="imr-intruder report")
    report.set_defaults(func=cmd_report)
    macro = sub.add_parser("macro", help="Run a multi-step authentication/request macro")
    macro.add_argument("file")
    macro.add_argument("--session")
    macro.add_argument("--output")
    macro.set_defaults(func=cmd_macro)
    websocket = sub.add_parser("websocket", help="Send controlled WebSocket messages")
    websocket.add_argument("url")
    websocket.add_argument("--message", action="append")
    websocket.add_argument("--messages-file")
    websocket.add_argument("--header", action="append", default=[])
    websocket.add_argument(
        "--timeout", type=float_range(0.0, 300.0, exclusive_minimum=True), default=10
    )
    websocket.set_defaults(func=cmd_websocket)
    browser = sub.add_parser("browser", help="Render a page using optional Playwright")
    browser.add_argument("url")
    browser.add_argument("--screenshot")
    browser.add_argument(
        "--timeout", type=float_range(0.0, 600.0, exclusive_minimum=True), default=30
    )
    ...
    collab = sub.add_parser("collab", help="Manage multiuser web tokens")
    collab_sub = collab.add_subparsers(dest="collab_action", required=True)
    ...
    web = sub.add_parser("web", help="Manage the web console")
    web.add_argument(
        "web_action",
        nargs="?",
    ...
    web.add_argument("--port", type=int_range(1, 65535), default=7415)
    ...
    check = sub.add_parser("check-update", help="Check GitHub for a newer version")
    check.add_argument("--repository", default=DEFAULT_REPOSITORY)
    check.add_argument("--channel", choices=["release", "main"], default="release")
    check.add_argument("--token")
    check.add_argument("--json", dest="json_output", action="store_true")
    check.set_defaults(func=cmd_check_update)
    update = sub.add_parser("update", help="Download and install an update without git clone")
    update.add_argument("--repository", default=DEFAULT_REPOSITORY)
    update.add_argument("--channel", choices=["release", "main"], default="release")
    update.add_argument("--token")
    update.add_argument("--dry-run", action="store_true")
    update.add_argument("--force", action="store_true")
    update.set_defaults(func=cmd_update)
    doctor = sub.add_parser("doctor", help="Validate runtime and configured paths")
