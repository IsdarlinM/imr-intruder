...
def rule_matches(spec: str, result: dict[str, Any]) -> bool:
    ...
    if kind == "header":
        ...
        actual = str(result.get("response_headers", {}).get(name, ""))
...
def extract_value(spec: str, result: dict[str, Any]) -> str:
    ...
    body = result.get("body_preview", "")
    if kind == "header":
        return str(result.get("response_headers", {}).get(value, ""))
    if kind == "regex":
...
def enrich_results(
    ...
    baseline = results[min(max(baseline_index, 0), len(results) - 1)]
    ...
    for result in results:
        ...
        result["body_hash"] = digest
        result["similarity"] = similarity(baseline_normalized, normalized)
        result["delta_bytes"] = result.get("size_bytes", 0) - baseline.get("size_bytes", 0)
    ...
    sizes = [float(item.get("size_bytes", 0)) for item in results]
    times = [float(item.get("elapsed_ms", 0)) for item in results]
