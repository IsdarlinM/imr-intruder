...
def run_macro(path: Path, session_name: str | None = None) -> list[dict[str, Any]]:
    ...
    session = load_session(session_name) if session_name else {"headers": {}, "cookies": {}, "variables": {}}
    ...
    for index, step in enumerate(steps, start=1):
        ...
        request["headers"] = {**session.get("headers", {}), **request.get("headers", {})}
        request["cookies"] = {**session.get("cookies", {}), **request.get("cookies", {})}
        ...
        if step.get("require_status") is not None and result.get("status") != int(step["require_status"]):
            raise RuntimeError(f"Macro step {index} returned {result.get('status')} instead of {step['require_status']}.")
    if session_name:
