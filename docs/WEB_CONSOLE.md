...
3. The browser sends JSON to `POST /api/jobs` with `X-Request-Token`.
...
6. Results stream as NDJSON from `GET /api/jobs/{job_id}/events`.
...
8. The completed or cancelled job can be exported with `GET /api/jobs/{job_id}/csv`.

## Controls

| Control | Operation |
|---|---|
| Run scan | Validates input, creates one job, disables duplicate submission, streams results, and restores controls on completion or error. |
...
| Analysis tab | Configures custom columns, match rules, exclude rules, and extraction rules. Invalid regular expressions are rejected before requests begin. |
...
| Result row | Opens the right-side evidence drawer. |
